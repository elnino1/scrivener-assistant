...
scrivener-assistant/
├── src/
│   ├── server.py                        # MCP entry point — 38 registered tools
│   └── scrivener_assistant/
...
│   └── test_integration_native_status.py
│
├── .claude/
...
class ScrivenerProject:
    ...
    draft_root: Optional[BinderNode]   # the DraftFolder node; None if absent
...
Only `Text`-type nodes are included. Folder, Research, Trash nodes are excluded. `"status"` is the native Scrivener status (resolved from `<StatusID>`), or `null` if unset.
...
| `get_binder_structure` | `()` | Return the DraftFolder subtree as JSON. Non-manuscript sections (Characters, Notes, Research, Trash, etc.) are excluded. |
...
| `list_native_statuses` | `()` | Return all configured Scrivener status labels (excluding "No Status") as a newline-separated string. Call this before `update_native_status` to verify which labels are valid. |
| `update_native_status` | `(uuid, status)` | Set the native Scrivener `<StatusID>` for a scene. Returns an error string if the label is not in `<StatusSettings>`. Triggers registry rebuild. |
...
- Coverage is validated by running `pytest tests/` — all tests must pass before any merge.
...
| **`.scrivx` write scope** | Only `<CustomMetaData>` and `<MetaData><StatusID>` fields are written. Scrivener's core structure (binder, titles, content links) is never touched. |
...
| **DraftFolder scoping** | `scene_registry.json` and `get_binder_structure` only include nodes under the `DraftFolder`. Characters, Places, Notes, Front Matter, Research, Template Sheets, and Trash are excluded. Direct UUID tools (`read_document`, `update_metadata`, etc.) remain unrestricted. |
